...
class ConnectCore {
  ...
  requestCredentials(request = {
    ...
    return new Promise((resolve, reject) => {
      if (this.canSign) {
        this.credentials.createRequest({
          ...request,
  ...
  attestCredentials({sub, claim, exp}, uriHandler) {
    ...
    return this.credentials.attest({ sub, claim, exp }).then(jwt => {
      return self.request({
        uri: `me.uport:add?attestations=${encodeURIComponent(jwt)}&callback_url=${encodeURIComponent(topic.url)}`,
        topic, uriHandler
      });
    });
  }
  ...
  */
  ...
  sendTransaction(txobj, uriHandler) {
    ...
    const uri = paramsToUri(this.addAppParameters(txobj, topic.url));
